...
class Titan:
    ...
    def error_handler(self, fn: ErrorHandler) -> ErrorHandler:
        ...
        إذا لم يُسجَّل error handler، يُسجَّل الخطأ عبر logger الخاص بـ Titan
        مع traceback كامل.
        استثناء داخل error handler نفسه يُسجَّل بالطريقة نفسها ولا يُسكَت.
        يوجد موضع واحد لمعالج الأخطاء؛ التسجيل الأخير يستبدل السابق
        عمداً، من دون تحذير أو استثناء.
    ...
    async def run_async(
        ...
        def _offset_updated(new_offset: int) -> None:
            self.offset = new_offset
            if on_offset is not None:
                on_offset(new_offset)
        ...
        if current_task is not None:
            signals.install(loop, current_task)

        try:
            await runner.run(
                initial_offset=self.offset,
                debug=debug,
                offset_updated=_offset_updated,
            )
        finally:
            if current_task is not None:
