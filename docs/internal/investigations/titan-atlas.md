...
**الحالة:** مُغلقة — القرار المعماري اتُّخذ — راجع ADR-014 (Titan Atlas)
**تاريخ:** 2026-07-11
**الميزة:** Titan Atlas (#9) — من قائمة Planned

---

## 1. المشكلة التي تحلها Titan Atlas
...
هذا يعني: Titan Atlas لا تحتاج معرفة خارجية — **المعرفة موجودة ومُهيكلة بالفعل**.

---

## 3. ما هي "Titan Atlas" بالضبط — وما ليست هي

### ما هي:

طبقة المعرفة المعمارية لـ Titan (`titan.atlas`) — تُجيب على أسئلة حول Titan نفسها: لماذا اتُّخذت قرارات، ما الفلسفة، هل تغيير مقترح يتعارض مع ADR.
...
الخيارات:
- **`titan.atlas`** — domain منفصل مثل `titan.playground` و`titan.profiler`
...
```python
from titan.atlas import explain
explain("feed_update")
...
```python
from titan.atlas import decisions
...
```python
from titan.atlas import search
...
**السؤال ب:** هل تعيش في `titan.atlas` (domain منفصل)؟
...
**ب. `titan.atlas` — domain منفصل (الاسم الرسمي: Titan Atlas).**
لا يُلحق بـ `bot`. Bot هو محرك التنفيذ — Titan Atlas مستهلكة لأنظمة المعرفة.
