...
**Titan Atlas** — طبقة المعرفة المعمارية لـ Titan (`titan.atlas`).
...
**`titan.atlas` — domain منفصل، rule-based، يستهلك `titan.timeline`.**
...
`titan.atlas` لا يُلحق بـ `bot`. Bot محرك تنفيذ — Titan Atlas مستهلكة لأنظمة المعرفة.
...
**Titan Atlas تُلخّص ما يعرفه Titan بالفعل — محددة النتائج في v1، LLM اختياري لاحقاً.**
...
### إلحاق Titan Atlas بـ `bot` كـ `bot.explain()` / `bot.ask()`
