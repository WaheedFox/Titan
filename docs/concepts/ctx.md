...
`ctx.permissions` tells you what the bot is permitted to do in the current chat. But `ctx` does not populate this automatically on every update — that would mean an API call on every handler, even when you have no intention of checking permissions.

Instead, you call `ctx.fetch_permissions()` explicitly when you need that information. The result is stored in `ctx.permissions` for the duration of the handler.
