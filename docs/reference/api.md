...
| `ctx.new_members` | `list[dict] \| None` | List of raw user dicts for users who joined; available in `new_member` handlers |
| `ctx.left_member` | `dict \| None` | Raw user dict for the user who left; available in `left_member` handlers |
...
| `ctx.can_delete` | `bool \| None` | `None` until `ctx.refresh_permissions()` is called |
...
Fetches the bot's current permissions in the chat and stores the result in `ctx.can_delete`. Makes one API call. Call it explicitly when you need to check deletion rights.
